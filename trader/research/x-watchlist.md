...
**⚠ 本周 X 登录态丢失。** `x.com/home` 返回「已有账号? 登录」页而非时间线，`querySelectorAll('article')` 无从谈起。
按手册 §6：**这是登录态问题，不是账号问题**；本周改用 WebSearch 与一手源补齐，并在周报里提醒用户重登（账号 @buliuyihanchong）。
**在重登之前，本清单的所有打分只能来自转述与可复算的价格，不能来自原帖。**

### 到期打分（价格取自 `engine.py close`，日线已完成 K 线）

| 账号 | 上周挂的断言 | 本周实测 | 判定 | 分数 |
|---|---|---|---|---|
| **@fit_businessman** | 「霍尔木兹封锁在特朗普任内一律利多股市，他会安排周日拉盘轧空」——**周末美伊直接交火 = 天然实验** | SPY 本周 **−0.77%**，^GSPC −0.80%，没有轧空 | ❌ **明确证伪**。这是一条被反复主张、终于等到干净实验的断言 | **5 → 4** |
| **@spotgamma** | 「Downside? No one cares」，call skew 高于 put skew，跌保护极便宜 | 本周最大回撤仅 −1.60%；SPY put skew rank 最新 **2.8**（近一年极低） | ✅ **状态读数连续两周成立** | **5.5 → 6** |
| @spotgamma | SMH/AVGO/MU call skew >80 分位 → 两周内 AI 复合体 >5% 回撤 | SMH 两周最大回撤约 −1.75%，未达标 | ❌ 条件到期未兑现 | 不加分 |
| **@aleabitoreddit（Serenity）** | 存储瓶颈未变、看多存储链（连续两周兑现） | **MU −4.07%、SNDK −6.13%、WDC −4.34%、STX −2.25%** | ❌ 第三周反转 | **7 → 6.5** |

**Serenity 这一条要按机制记，不能只按对错记**：反转的原因是 **DeepSeek V4.1-Flash（9/10 发布）把 KV Cache 的 HBM 需求压到 1/4、SSD 压到 1/8**，9/11 三星与 SK 海力士在首尔跌超 3%。
他的供需框架没错，**是这条链上突然多了一个他不跟踪的变量：模型侧的内存效率**。→ **已把「单位算力的内存需求」加进我自己的监控项**；这类反转按「框架缺一个输入」处理，扣 0.5 而不是打回原形。

### 清单变动

- **降级**：@fit_businessman 5 → **4**（事件驱动的方向预测被干净证伪，此后只读不计分）。
- **维持「每次必读」**：@aleabitoreddit（6.5）、@DeItaone（6）。
- **升级**：@spotgamma 5.5 → **6**，但**用法收窄为「只取定位/状态读数，不取方向预测」**，与主任只取点位卡是同一种分层用法（§5.1 家族）。
- **新增候选（下周开始跟踪）**：
  - **@DeepSeek_AI 官方 / @teortaxesTex** —— 本周存储股的真驱动来自模型架构侧，而我的清单里**一个跟踪模型效率的账号都没有**。这是一个被证明过会直接打穿半导体板块的变量，必须补上。
  - **@NickTimiraos** —— 本周社长与好奇先生**两条独立转述**都指向他，说明他是这一轮利率叙事的上游。与其读两道转述，不如直接读源头。
- **暂不新增**：A 股方向仍走雪球热榜 + 券商策略周报。

### 读法备忘（登录恢复后照旧）

先 `resize_window` 到 1280×1600 → navigate → 等 9 秒 → `javascript_tool: Array.from(document.querySelectorAll('article')).slice(0,14).map(a=>a.innerText.replace(/\n+/g,' | ').slice(0,400)).join('\n---\n')`。
`get_page_text` 对 X 时间线返回空，这一点本周再次确认（登录页倒是读得到）。
